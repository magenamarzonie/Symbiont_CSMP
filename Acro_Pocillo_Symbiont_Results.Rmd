...
Ues the gt package. 
```{r}
#maybe follow this tutorial in a separate data sheet 
library(gt)   #gt package to create publication quality tables 
```
...
-is PSba diversity congruent iwth profile diversity? 

## Number of profiles total
## Profile congruence with P. veruucosa ID (what are the exceptions)
...
```{r}
#Tree + sequences for all samples 
p_bar_uni + p_tree_tip
```
...
## Permanova
#MM: This is currently just for p.verrucosa and also it is not working 
```{r}
library(vegan)
...
```
#Unifrac distances between profiles ? (Hume 2020; Fig6)
