...
car::vif(lm(1:nrow(adonis_meta_pver) ~ GPS_S + Aspect + Exposure + catBleaching + Depth + DHW, data=adonis_meta_pver))
...
res.ad <- adonis3(dist_pver ~ catBleaching, data = adonis_meta_pver, permutations = 999)
...
```{r}
#first try adonis3 function
adonis3(as.dist(du_pver) ~ GPS_S + GPS_E + DHW3 + DHW6 , data = adonis_meta_pver)
adonis3(as.dist(du_pver) ~  DHW3 + DHW6 + DHW9, data = adonis_meta_pver)
...
##dbRDA 
```{r}
#capscale function for analysing unifrac distance with RDA 

pver.cap <- capscale(as.dist(du_pver) ~ Reef, data = adonis_meta_pver)
pver.cap
```
...
pver.cap <- capscale(as.dist(du_pver) ~ Reef + Depth, data = adonis_meta_pver)
pver.cap
```
