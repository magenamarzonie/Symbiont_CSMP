...
adonis_meta_pver <- full_meta2 %>%
  dplyr::select(plate_position, Sector, Reef.x, Site, Species, catBleaching, Depth, DHW, GPS_S = `GPS south`, GPS_E = `GPS east`, Aspect, Exposure, maxDHW, meanDHW, recent.maxDHW, recent.meanDHW, DHW2, DHW3, DHW4, DHW6, DHW8, DHW9, rangeSST, varSST, MMM) %>%
  filter(plate_position %in% rownames(du_pver)) %>%
  tibble::column_to_rownames(var = "plate_position") %>%
  mutate(sample_name = rownames(.))

#1: test VIF scores of all variables 
car::vif(lm(1:nrow(adonis_meta_pver) ~  Aspect, catBleaching, GPS_S, Reef.x, Site, Depth, Exposure, maxDHW, meanDHW, recent.maxDHW, recent.meanDHW, DHW2, DHW3, DHW4, DHW6, DHW8, DHW9, returnDHW3, returnDHW4, returnDHW6, rangeSST, varSST, MMM, data=adonis_meta_pver))


#Which variables have high VIF scores? 
#We removed X and Y variables to remove any variables with high co-linearity 
```
