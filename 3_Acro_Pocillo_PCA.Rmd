...
## 3.1a Permanova Pocillopora all species 
```{r}
...
res <- adonis(dist_poci ~ DHW3, data = adonis_meta_poci)

test_poci <- betadisper(as.dist(du_poci), adonis_meta_poci$DHW3)
plot(test_poci)
anova(test_poci)
```
...
ggplot(pcoa_df_poci, aes(Axis.1, Axis.2)) +
  ...
  ggalt::geom_encircle(aes(group = Species, fill = Species), s_shape=1, expand=0, alpha = 0.1) 
```
...
ggplot(pcoa_df_acro, aes(Axis.1, Axis.2)) +
  geom_point(aes(fill = DHW3), shape = 21, size = 3, alpha = 0.8) +
  ggalt::geom_encircle(aes(group = DHW3, fill = DHW3), s_shape=1, expand=0, alpha = 0.1) 
```
