...
full_meta2 = full_meta2 %>% 
  mutate(DHW = as.numeric(DHW),
         ...
         catBleaching = as.numeric(catBleaching),
...
ord_pver <- dbrda(dist_pver ~ Depth + DHW3 + DHW6 + recent.maxDHW + catBleaching + DHW2020 + varSST + GPS_S, data=adonis_meta_pver)

# Re check the new vif scores
sort(vif.cca(ord_pver))
  # Depth          DHW3 recent.maxDHW          DHW6         GPS_S        varSST 
  #    1.076655      1.314088      1.626934      1.784354      2.039474      2.938100 
...
cm <- cor(adonis_meta_pmea %>% select(Depth, maxDHW, meanDHW, recent.maxDHW, 
                                      recent.meanDHW, DHW3, DHW4, DHW6, DHW8, 
                                      DHW9, rangeSST, varSST, MMM, GPS_S, catBleaching, DHW2020))
library(corrplot)
...
# Reduce the model (consult the corplot and vif scores)
ord_pmea <- dbrda(dist_pmea ~ Depth + DHW3 + DHW6 + recent.maxDHW + catBleaching + DHW2020 + varSST + GPS_S, data = adonis_meta_pmea)
...
os_pmea_gps <- ordistep(ord_pmea_gps, direction = "backward")
anova(os_pmea_gps, by = 'margin')
plot(os_pmea_gps)

os_pmea_dhw <- ordistep(ord_pmea_dhw, direction = "backward")
anova(os_pmea_dhw, by = 'margin')
plot(os_pmea_dhw)
...
# Reduce the model (consult the corplot and vif scores)
ord_punk <- dbrda(dist_punk ~ Depth + DHW3 + DHW6 + recent.maxDHW + catBleaching + DHW2020 + varSST + GPS_S , data=adonis_meta_punk)
...
cm_acro <- cor(adonis_meta_acro %>% select(Depth, maxDHW, meanDHW, recent.maxDHW, 
                                      recent.meanDHW, DHW3, DHW4, DHW6, DHW8, 
                                      DHW9, rangeSST, varSST, MMM, GPS_S, DHW2020, catBleaching))
library(corrplot)
...
# Reduce the model (consult the corplot and vif scores)
ord_acro <- dbrda(dist_acro ~ Depth + DHW3 + DHW6 + recent.maxDHW + varSST + GPS_S +DHW2020 + catBleaching, data=adonis_meta_acro)

ord_acro_gps <- dbrda(dist_acro ~ Depth + DHW3 + DHW6 + recent.maxDHW + varSST + GPS_S + catBleaching, data=adonis_meta_acro)

ord_acro_dhw <- dbrda(dist_acro ~ Depth + DHW3 + DHW6 + recent.maxDHW + varSST + DHW2020 + catBleaching, data=adonis_meta_acro)
...
Fig3 = ggarrange (g1 + g2 + g3 + g4, nrow = 1) + 
    ...
    guides(fill = guide_colourbar(title.position = "right", barheight = 17, barwidth = .8))
