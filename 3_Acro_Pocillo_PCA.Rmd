...
## 3.1a Permanova Pocillopora Verrucosa 
```{r}
...
res <- adonis(dist_poci ~ Species, data = adonis_meta_poci)

test_poci <- betadisper(as.dist(du_poci), adonis_meta_poci$Species)
plot(test_poci)
anova(test_poci)
```
...
test_acro <- betadisper(as.dist(du_acro), adonis_meta_acro$Reef)
plot(test_acro)
anova(test_acro)
```
